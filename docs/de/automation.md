...
Netzwerkautomatisierung reduziert manuelle Fehler, spart Zeit und gewährleistet Konsistenz über Ihre Infrastruktur hinweg. Nachdem Sie diese Laborumgebung manuell erkundet haben, können Sie beginnen, häufige Aufgaben zu automatisieren wie:
...
Dieser Leitfaden zeigt Ihnen ein Beispiel, wie Sie mit dem vollständig Open-Source-Werkzeug [networka](https://narrowin.github.io/networka/) beginnen können, einem Multi-Vendor-Netzwerkautomatisierungs-CLI-Toolkit.
...
Dies erstellt ein Konfigurationsverzeichnis unter `~/.config/networka` (Linux/macOS) mit:
...
Folgen Sie den Eingabeaufforderungen, um Shell-Vervollständigungen und optionale Herstellersequenzen zu installieren.
...
## Schritt 4: Ihren ersten Befehl ausführen

Überprüfen Sie den Schnittstellenstatus auf dem Router mit einer vordefinierten Sequenz:

```bash
nw run router1 interface_status
```

Dies verbindet sich mit dem Gerät und führt mehrere Befehle aus, um zu zeigen:

- Alle Schnittstellen mit ihrem Status und ihrer Konfiguration
...
Die Ausgabe ist formatiert und leicht zu lesen, mit klaren Abschnitten für jedes Befehlsergebnis.
...
Führen Sie Befehle gleichzeitig auf mehreren Geräten aus:

```bash
# Kommagetrennte Liste
nw run router1,switch1 quick_status

# Einen Befehl über Geräte ausführen
nw run router1,switch1 "/system/resource/print"
```

Für den Produktionseinsatz erstellen Sie Gerätegruppen und führen Sie Befehle effizient über ganze Gruppen aus.
